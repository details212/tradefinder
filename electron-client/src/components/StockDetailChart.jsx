/**
 * StockDetailChart — Highcharts Stock chart for the Symbol Search detail panel.
 * Full feature parity with ModalChart (R/R drawing, live quote, order placement,
 * zoom presets, drag stop/target) but self-measures its container height instead
 * of receiving it as a prop, and treats barTime / threshold / bias as optional.
 *
...
const TIMEFRAMES = [
  { label: "5m",  multiplier: 5,  timespan: "minute", days: 28,   limit: 3000 },
  { label: "10m", multiplier: 10, timespan: "minute", days: 28,   limit: 2000 },
  { label: "15m", multiplier: 15, timespan: "minute", days: 45,   limit: 2000 },
  { label: "30m", multiplier: 30, timespan: "minute", days: 60,   limit: 1000 },
  { label: "1h",  multiplier: 1,  timespan: "hour",   days: 120,  limit: 1000 },
  { label: "4h",  multiplier: 4,  timespan: "hour",   days: 180,  limit: 500  },
...
function buildOptions(ticker, ohlcv, barTimeMs, threshold, barMs = 5 * 60 * 1000) {
  ...
  const padPoints = Array.from({ length: PAD_BARS }, (_, i) => lastT + barMs * (i + 1));
    ...
    navigator: {
      ...
      xAxis: {
        labels: {
          ...
          formatter() { return etTime.dateFormat("%b %e", this.value); },
        },
      },
    },
...
export default function StockDetailChart({ ticker, barTime = null, threshold = null, bias = null }) {
  ...
  const [direction,     setDirection]     = useState(bias === "short" ? "short" : "long");
  ...
  const barMs = timeframe.timespan === "minute" ? timeframe.multiplier * 60 * 1000
              : timeframe.timespan === "hour"   ? timeframe.multiplier * 3600 * 1000
              : timeframe.timespan === "day"    ? timeframe.multiplier * 86400 * 1000
              :                                   timeframe.multiplier * 7 * 86400 * 1000;

  const paddedLastT = useCallback(() => {
    if (!bars.length) return 0;
    return bars[bars.length - 1].t + 30 * barMs;
  }, [bars, barMs]);
  ...
  // Clear R/R drawing when timeframe changes — old levels won't align with new bars
  useEffect(() => { setRr(null); setRrMode(false); }, [timeframe]);

  // When the container height changes (e.g. R/R panel or quote strip appears/disappears),
  // tell Highcharts to recalculate its internal dimensions. Without this, chart.yAxis[0].toPixels()
  // and chart.plotHeight keep using stale values, so SVG renderer elements are drawn at wrong
  // positions and appear "unanchored" from the price axis.
  useEffect(() => {
    const chart = chartRef.current?.chart;
    if (!chart || !chartHeight) return;
    chart.reflow();
  }, [chartHeight]);

  // ── Risk prefs + portfolio value ─────────────────────────────────────────────
  useEffect(() => {
    preferencesApi.get()
      .then(r => {
  ...
  // ── Initial zoom to signal bar (or preset) ───────────────────────────────────
  useEffect(() => {
    const chart = chartRef.current?.chart;
    if (!chart || !bars.length) return;
    chart.reflow();
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      ...
      const isLong   = directionRef.current === "long";
      const stop     = isLong
  ...
  // ── Re-clip + redraw on every render (zoom / scroll) ─────────────────────────
  useEffect(() => {
    const chart = chartRef.current?.chart;
    if (!chart) return;
    const onRender = () => {
  ...
  const chartOptions = useMemo(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [ticker, bars, barTimeMs, threshold, barMs]
  );
  ...
  return (
    <div className="relative bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden flex flex-col"
         style={{ height: 720 }}>

      {/* ── Loading / error overlays (absolute so wrapRef always occupies layout space) ── */}
      {(loading || chartHeight == null) && !error && (
        <div className="absolute inset-0 flex items-center justify-center gap-2 text-slate-400 z-10 bg-slate-800/80">
          <Loader2 className="w-5 h-5 animate-spin" />
          <span className="text-sm">Loading chart…</span>
        </div>
      )}
      {error && (
        <div className="absolute inset-0 flex items-center justify-center gap-2 text-red-400 z-10 bg-slate-800/80">
      ...
      {ready && (
        <>
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 border-b border-slate-800 bg-slate-900/60 shrink-0">
            ...
            {/* Timeframe selector */}
            <div className="flex items-center rounded overflow-hidden border border-slate-700 shrink-0 ml-1">
              {TIMEFRAMES.map((tf, i) => (
                <button
                  key={tf.label}
          ...
          {rr && (
            <div className="flex items-center gap-x-5 px-4 py-1.5 border-b border-slate-700/60 bg-slate-950/50 shrink-0 text-xs">
              {liveQuote && (!liveQuote.fetching || liveQuote.updatedAt) ? (
                <>
                  {liveQuote.bid != null && (
                    <span className="flex items-center gap-1">
                      <span className="text-slate-500">Bid</span>
                      <span className="font-mono font-semibold text-emerald-400">${liveQuote.bid.toFixed(2)}</span>
                      {liveQuote.bidSize != null && <span className="text-slate-600 text-[10px]">×{liveQuote.bidSize}</span>}
                    </span>
                  )}
                  {liveQuote.ask != null && (
                    <span className="flex items-center gap-1">
                      <span className="text-slate-500">Ask</span>
                      <span className="font-mono font-semibold text-red-400">${liveQuote.ask.toFixed(2)}</span>
                      {liveQuote.askSize != null && <span className="text-slate-600 text-[10px]">×{liveQuote.askSize}</span>}
                    </span>
                  )}
                  {liveQuote.spread != null && (
                    <span className="flex items-center gap-1">
                      <span className="text-slate-500">Spread</span>
                      <span className="font-mono text-yellow-400">${liveQuote.spread.toFixed(2)}</span>
                    </span>
                  )}
                  {liveQuote.last != null && (
                    <span className="flex items-center gap-1">
                      <span className="text-slate-500">Last</span>
                      <span className="font-mono text-slate-300">${liveQuote.last.toFixed(2)}</span>
                    </span>
                  )}
                  ...
                  <span className="ml-auto flex items-center gap-1.5 text-slate-600">
                    {liveQuote.fetching && <span className="w-1.5 h-1.5 rounded-full bg-brand-400 animate-pulse" />}
                    {liveQuote.updatedAt && (
                ...
                <div className="bg-slate-800 border border-slate-600 rounded-xl shadow-2xl w-72 overflow-hidden">
                    ...
                    {isMarket ? (
                      ...
                      <div className="flex justify-between items-center py-1.5 border-b border-slate-700/50">
                        ...
                        <span className="flex flex-col items-end gap-0.5">
                          <span className="font-mono text-xs font-semibold text-slate-200">${smartEntry.toFixed(2)}</span>
                  ...
                  <div className="flex gap-2 px-4 pb-4">
                    <button
                      ...
                      onClick={() => { setOrderType(null); setOrderResult(null); }}
      ...
      {/* ── Chart wrapper — always in DOM so ResizeObserver can measure it ── */}
      <div ref={wrapRef} className="relative flex-1 min-h-0">
        {ready && chartHeight && (
          <HighchartsReact
            ...
            containerProps={{ style: { height: `${chartHeight}px`, width: "100%" } }}
          />
        )}
      </div>
    </div>
