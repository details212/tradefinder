...
Results are written to `vcz`, keyed on (symbol, bar_date) so it
can be INNER JOIN-ed with bars_daily at any time.
...
# ── DDL ───────────────────────────────────────────────────────────────────────
DDL = """
CREATE TABLE IF NOT EXISTS vcz (
...
def ensure_table(conn):
    ...
    migrations = [
        "ALTER TABLE vcz ADD COLUMN rsi_cross_50       TINYINT(1)    NOT NULL DEFAULT 0    AFTER rsi_14",
        "ALTER TABLE vcz ADD COLUMN rsi_cross_down_50  TINYINT(1)    NOT NULL DEFAULT 0    AFTER rsi_cross_50",
        "ALTER TABLE vcz ADD COLUMN atr_10             DECIMAL(14,4) DEFAULT NULL        AFTER dc_lower",
        "ALTER TABLE vcz ADD COLUMN atr_squeeze        TINYINT(1)    NOT NULL DEFAULT 0 AFTER atr_10",
        "ALTER TABLE vcz ADD COLUMN pivot_high         DECIMAL(14,4) DEFAULT NULL        AFTER atr_squeeze",
        "ALTER TABLE vcz ADD COLUMN atr_declining_bars SMALLINT      DEFAULT NULL        AFTER pivot_high",
        "ALTER TABLE vcz ADD COLUMN atr_streak_len     SMALLINT      DEFAULT NULL        AFTER atr_declining_bars",
        "ALTER TABLE vcz ADD COLUMN atr_streak_ago     SMALLINT      DEFAULT NULL        AFTER atr_streak_len",
        "ALTER TABLE vcz ADD COLUMN as_raw_1m  DECIMAL(14,4)    DEFAULT NULL AFTER vcp_is_signal",
        "ALTER TABLE vcz ADD COLUMN as_raw_3m  DECIMAL(14,4)    DEFAULT NULL AFTER as_raw_1m",
        "ALTER TABLE vcz ADD COLUMN as_raw_6m  DECIMAL(14,4)    DEFAULT NULL AFTER as_raw_3m",
        "ALTER TABLE vcz ADD COLUMN as_raw_12m DECIMAL(14,4)    DEFAULT NULL AFTER as_raw_6m",
        "ALTER TABLE vcz ADD COLUMN as_1m      TINYINT UNSIGNED DEFAULT NULL AFTER as_raw_12m",
        "ALTER TABLE vcz ADD COLUMN as_3m      TINYINT UNSIGNED DEFAULT NULL AFTER as_1m",
        "ALTER TABLE vcz ADD COLUMN as_6m      TINYINT UNSIGNED DEFAULT NULL AFTER as_3m",
        "ALTER TABLE vcz ADD COLUMN as_12m     TINYINT UNSIGNED DEFAULT NULL AFTER as_6m",
        # Widen as_raw_* if already created with the narrower DECIMAL(10,4)
        "ALTER TABLE vcz MODIFY COLUMN as_raw_1m  DECIMAL(14,4) DEFAULT NULL",
        "ALTER TABLE vcz MODIFY COLUMN as_raw_3m  DECIMAL(14,4) DEFAULT NULL",
        "ALTER TABLE vcz MODIFY COLUMN as_raw_6m  DECIMAL(14,4) DEFAULT NULL",
        "ALTER TABLE vcz MODIFY COLUMN as_raw_12m DECIMAL(14,4) DEFAULT NULL",
        "ALTER TABLE vcz ADD COLUMN rmv_5  DECIMAL(10,4) DEFAULT NULL AFTER as_12m",
        "ALTER TABLE vcz ADD COLUMN rmv_15 DECIMAL(10,4) DEFAULT NULL AFTER rmv_5",
        "ALTER TABLE vcz ADD COLUMN rmv_50 DECIMAL(10,4) DEFAULT NULL AFTER rmv_15",
        "ALTER TABLE vcz ADD COLUMN rs_line     DECIMAL(14,6) DEFAULT NULL          AFTER rmv_50",
        "ALTER TABLE vcz ADD COLUMN rs_ema_21   DECIMAL(14,6) DEFAULT NULL          AFTER rs_line",
        "ALTER TABLE vcz ADD COLUMN rs_sma_50   DECIMAL(14,6) DEFAULT NULL          AFTER rs_ema_21",
        "ALTER TABLE vcz ADD COLUMN rs_new_high          TINYINT(1)    NOT NULL DEFAULT 0    AFTER rs_sma_50",
        "ALTER TABLE vcz ADD COLUMN rs_cross_above_sma50 TINYINT(1)    NOT NULL DEFAULT 0    AFTER rs_new_high",
        "ALTER TABLE vcz ADD COLUMN rs_cross_below_sma50 TINYINT(1)    NOT NULL DEFAULT 0    AFTER rs_cross_above_sma50",
    ]
...
def fetch_ohlcv(conn, symbol: str) -> list:
    ...
    cur.execute(
        """
        SELECT DATE(bar_time) AS bar_date, open, high, low, close, volume
        FROM   bars_daily
        WHERE  ticker = %s
        ORDER  BY bar_time
        """,
...
def fetch_benchmark(conn, symbol: str = RS_BENCHMARK) -> dict:
    ...
    cur = conn.cursor(dictionary=True)
    cur.execute(
        "SELECT DATE(bar_time) AS bar_date, close FROM bars_daily WHERE ticker = %s ORDER BY bar_time",
        (symbol,),
    )
    rows = cur.fetchall()
    cur.close()
...
def delete_symbol(conn, symbol: str):
    cur = conn.cursor()
    cur.execute("DELETE FROM vcz WHERE symbol = %s", (symbol,))
    conn.commit()
    cur.close()
...
def run(symbol: str, replace: bool, silent: bool = False,
    ...
    try:
        ...
        if not rows:
            conn.close()
            msg = f"no rows found in bars_daily for symbol={symbol!r}"
            if not silent:
                print(f"  ERROR: {msg}")
                sys.exit(1)
        ...
        if not silent:
            print()
            print("  Sample query:")
            print(f"    SELECT DATE(o.bar_time) AS bar_date, o.close, s.sma_50, s.rsi_14, s.vcp_score")
            print(f"    FROM   bars_daily o")
            print(f"    INNER  JOIN vcz s ON s.symbol=o.ticker AND s.bar_date=DATE(o.bar_time)")
            print(f"    WHERE  o.ticker = '{symbol}'")
            print(f"    ORDER  BY o.bar_time;")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Calculate technical studies and save to vcz.\n"
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "from the as_raw_* values already stored in vcz."
        ),
    )
    parser.add_argument(
        "--sleep",
        ...
        metavar="MINUTES",
        help=(
            "Loop indefinitely, sleeping MINUTES between runs. "
    ...
    while True:
        ...
        next_run = time.strftime("%H:%M:%S", time.localtime(time.time() + args.sleep * 60))
        print(f"\nSleeping {args.sleep} minute(s) — next run at {next_run}. Press Ctrl+C to stop.")
